...
+ (void)load{
    ...
    dispatch_once(&onceToken, ^{
        ...
        [self swizzSelector:@selector(keyBoardTouchesEnded:withEvent:) originalSelector:@selector(touchesEnded:withEvent:)];
        [self swizzSelector:@selector(keyBoardSetFrame:) originalSelector:@selector(setFrame:)];
        [self swizzSelector:@selector(keyBoardSetContentSize:) originalSelector:@selector(setContentSize:)];
    });

}


-(void)keyBoardSetFrame:(CGRect)frame {
    ...
    [self keyBoardSetContentSize:contentSize];
    

}

-(void)keyBoardSetContentSize:(CGSize)contentSize {
    contentSize.width = MAX(contentSize.width, self.frame.size.width);
    contentSize.height = MAX(contentSize.height, self.frame.size.height);
    [self keyBoardSetContentSize:contentSize];
}

+ (void)swizzSelector:(SEL)swizzledSelector originalSelector:(SEL)originalSelector{
...
-(void)adjustOffsetIfNeeded
{
    ...
    CGFloat visibleSpace =  self.keyBoardScrollView().bounds.size.height -  self.keyBoardScrollView().contentInset.top -  self.keyBoardScrollView().contentInset.bottom;
    
    CGPoint idealOffset = CGPointMake(0, [self idealOffsetForView:[ self.keyBoardScrollView() findFirstResponderBeneathView: self.keyBoardScrollView()] withSpace:visibleSpace]);
    
    [ self.keyBoardScrollView() setContentOffset:idealOffset animated:YES];
}
